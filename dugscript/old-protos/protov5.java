/*
  Language Interpreter Prototype 5: Codename "Loopty-Loop"
*/
...
public class protov5 {
	...
	private static String eval(String[] cmds) {
		...
		if (intern > 0 && !(s.equals(";"))) {
		    //System.out.println("Added to if stack: " + s);
		    terncmds.add(s);
		    continue;
		}
		if (intern > 0 && s.equals(";")) {
		    //System.out.println("Added to if stack, time to eval: " + s);
		    terncmds.add(s);
		...
		*/
		...
		case "for":
		    ...
		    //I think I might decrement i once more
		    //so the stack is cleared after the foreval
		    //stack[i] = foreval(stack[i], stack[i + 1]);
	...
	*/
	...
	for (String str : arraccess) {
		...
		if (revsplit[i].equals("loc")) {		    
		    revsplit[i] = str;
		    //System.out.println("loc is now: " + revsplit[i]);
		}
